...
if __name__ == '__main__':
    ...
    for quality in threshold:
        ...
        for i in range(len(_data)):
            ...
            adv_img,perturb = trainer.attacker.perturb(img,epsilon=1,save_perturb=save_path_perturb)
            ...
            diff = np.clip(adv_img_ - ori_img_,0,255)
            perturb = np.clip(1.1*perturb, 0, 255)
            cv2.imwrite('dlib_adv.png',cv2.cvtColor(adv_img_.transpose((1,2,0)), \
                cv2.COLOR_RGB2BGR))
            rgbImg = cv2.imread('dlib_adv.png')
            rgbImg = rgbImg.transpose((2,1,0))
            # cv2.imwrite(save_path_perturb,perturb)
            # jpg_adv_img_ = img2jpg(adv_img_,im_path_clone.split('/')[-1],quality)
            adv_bboxes, adv_labels, adv_scores = trainer.faster_rcnn.predict([rgbImg], \
            ...
            fig2.savefig(save_path_adv,bbox_inches='tight',pad_inches=0)
            fig3, (ax3) = plt.subplots(1, 1, sharey=True,
                    sharex=True)
            ax3.axis('off')
            ax3.imshow(diff.astype(np.uint8))
            fig3.savefig(save_path_perturb,bbox_inches='tight',pad_inches=0)
            # ori_img_ = ori_img_.transpose((1,2,0))
            # adv_img_ = adv_img_.transpose((1,2,0))
